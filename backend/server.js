...
const demoUsers = [
  ...
  },
  {
    ...
    roleId: '1',
    roleName: 'Admin',
    permissions: ['tickets.create', 'tickets.edit', 'tickets.delete', 'tickets.message', 'system.management', 'system.settings', 'system.ai_settings', 'system.user_management', 'system.role_management', 'users.create', 'users.edit', 'users.delete', 'audit.view', 'insights.view', 'customers.view', 'devices.view', 'devices.create', 'devices.edit', 'devices.delete', 'companies.view', 'companies.create', 'companies.edit', 'companies.delete'],
    isActive: true,
    agentStatus: 'online',
  ...
  },
  {
    ...
    roleId: '2',
    roleName: 'Tier2',
    permissions: [], // Will be synced from role configuration
    isActive: true,
    agentStatus: 'online',
...
app.post('/api/tickets', (req, res) => {
  ...
  if (!user && (!customerInfo || !customerInfo.name || !customerInfo.email)) {
    console.log('âŒ Validation failed - customer info required for anonymous users');
    return res.status(400).json({
      success: false,
...
app.get('/api/roles', authenticateToken, (req, res) => {
  ...
  try {
    // Check permission - only allow role management and admin
    if (!req.user.permissions || !(req.user.permissions.includes('system.role_management') || req.user.roleName === 'Admin')) {
...
app.get('/api/agents', authenticateToken, (req, res) => {
  try {
    // Check permission - only allow system user management and admin
...
// Use enhanced AI routes with authentication
app.use('/api/ai', (req, res, next) => {
  // Apply authentication to enhanced AI routes
